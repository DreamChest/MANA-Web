class TagsController < ApplicationController
  ...
  before_action :set_tags, only: %i[index create update destroy clean]
    ...
    respond_to do |format|
      format.html do
        flash.now[:notice] = I18n.t(
          'notices.tags_cleaned',
          del_count: del_count,
          del_string: t('words.deleted_m', count: del_count).downcase
        )
        render :index
      end
    end
